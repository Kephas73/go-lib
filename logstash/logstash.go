...
func NewList(listAddr []string, timeout int) []*Logstash {
    ...
    for _, addr := range listAddr {
        ...
        host, port, err := util.ParseURL(addr)
        if err != nil {
            logger.Error("Address invalid: %s, error: %+v", addr, err)
...
func (l *Logstash) Writeln(message string) error {
    ...
    if l.Connection != nil {
        ...
        if err != nil {
            logger.Error("Gostash::Writeln - Error: %+v", err)
            ...
            _, conErr := l.Connect()
            logger.Error("Gostash::Writeln - Re-Connecting to [%+v] conErr: %+v", myAddr, conErr)

            return conErr
        }

        logger.Info("Gostash::Writeln - Connection write n: %d", n)
